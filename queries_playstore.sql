...
SELECT app_name, SUM(reviews) AS total_reviews FROM analytics GROUP BY app_name ORDER BY total_reviews DESC LIMIT 5; 

-- 5. Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT app_name,  SUM(reviews) AS total_reviews FROM analytics WHERE rating >= 4.8 GROUP BY app_name ORDER BY total_reviews DESC LIMIT 1; 
...
SELECT app_name, reviews, price FROM analytics WHERE price < 1.00 AND price > 0.10 ORDER BY reviews DESC LIMIT 10; 
...
SELECT app_name, last_updated AS last_update_date FROM analytics ORDER BY last_updated ASC LIMIT 1;

-- 12. Find the most expensive app (the query is very similar to #11).
SELECT app_name, last_updated AS last_update_date FROM analytics ORDER BY last_updated ASC LIMIT 1;

-- 13. Count all the reviews in the Google Play Store.
SELECT SUM(reviews) AS total_all_reviews FROM analytics; 
...
SELECT app_name, reviews, min_installs, (min_installs::FLOAT / reviews) AS proportion FROM analytics WHERE min_installs >= 100000 AND reviews > 0 ORDER BY proportion DESC LIMIT 1;